...
The LLM sees the peer list in its context, resolves "NUC11" to the URL, and calls `agent_call("http://192.168.1.101:8767", "!model")`.
...
mysql -u $MYSQL_USER -p$MYSQL_PASS mymcp \
  -e "INSERT INTO swarm_agents (name, url, model, description)
      VALUES ('NUC11', 'http://192.168.1.101:8767', 'qwen25', 'Local 7B model')
...
`!agent call NUC11 !model` expands to `agent_call("http://192.168.1.101:8767", "!model")` internally, removing the need to know or type the URL.
...
```
Use agent_call to ask the agent at http://192.168.1.101:8767 to run !model
```
