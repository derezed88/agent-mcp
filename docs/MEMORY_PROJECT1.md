...
| P4 | Pre-age topic normalization | **Built (2026-03-15)** | Med | `_age_topic_chunks()` Step 2c — fuzzy-normalizes candidate topic slugs (via `_normalize_topic()`) before aging to LT. Closes the race condition where ST hits HWM before `memreview_auto` runs, preventing fragmented slugs from being promoted verbatim. No LLM call — pure string matching. Updates both MySQL ST rows and Qdrant payloads atomically. Deduplicates candidates that normalize to the same slug. Scheduled `memreview_auto` handles semantic duplicates that fuzzy matching misses. |
